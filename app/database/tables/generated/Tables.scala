...
trait Tables {
  ...
  lazy val schema: profile.SchemaDescription = ArticleComments.schema ++ Articles.schema ++ UserProfilePicture.schema ++ Users.schema
  ...
  /** Entity class storing rows of table UserProfilePicture
   *  @param id Database column id SqlType(INT), AutoInc, PrimaryKey
   *  @param src Database column src SqlType(TEXT)
   *  @param userId Database column user_id SqlType(INT)
   *  @param createdOn Database column created_on SqlType(DATETIME) */
  case class UserProfilePictureRow(id: Int, src: String, userId: Int, createdOn: DateTime)
  /** GetResult implicit for fetching UserProfilePictureRow objects using plain SQL queries */
  implicit def GetResultUserProfilePictureRow(implicit e0: GR[Int], e1: GR[String], e2: GR[DateTime]): GR[UserProfilePictureRow] = GR{
    prs => import prs._
    UserProfilePictureRow.tupled((<<[Int], <<[String], <<[Int], <<[DateTime]))
  }
  /** Table description of table USER_PROFILE_PICTURE. Objects of this class serve as prototypes for rows in queries. */
  class UserProfilePicture(_tableTag: Tag) extends profile.api.Table[UserProfilePictureRow](_tableTag, Some("BLOGGING"), "USER_PROFILE_PICTURE") {
    def * = (id, src, userId, createdOn) <> (UserProfilePictureRow.tupled, UserProfilePictureRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = (Rep.Some(id), Rep.Some(src), Rep.Some(userId), Rep.Some(createdOn)).shaped.<>({r=>import r._; _1.map(_=> UserProfilePictureRow.tupled((_1.get, _2.get, _3.get, _4.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))

    /** Database column id SqlType(INT), AutoInc, PrimaryKey */
    val id: Rep[Int] = column[Int]("id", O.AutoInc, O.PrimaryKey)
    /** Database column src SqlType(TEXT) */
    ...
    /** Foreign key referencing Users (database name USER_PROFILE_PICTURE_USERS_user_id_fk) */
    lazy val usersFk = foreignKey("USER_PROFILE_PICTURE_USERS_user_id_fk", userId, Users)(r => r.userId, onUpdate=ForeignKeyAction.NoAction, onDelete=ForeignKeyAction.NoAction)
  }
  /** Collection-like TableQuery object for table UserProfilePicture */
  lazy val UserProfilePicture = new TableQuery(tag => new UserProfilePicture(tag))
